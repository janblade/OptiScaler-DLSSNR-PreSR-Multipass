...
PR #27 merged to main (`ce826161`) and shipped in release v0.1.9-auto-exposure-upstream-ports. Ledger `wilsjo2-fork:pr-77-auto-exposure-trim` closed as `ported`. Never run in a game: still to check are an HDR-linear NR game on D3D12 and one on Vulkan with source 3, NR toggled at a fixed spot (exposure must not chase its own output), Add/Preview anchors, and that Game exposure and Scanned exposure look as before.
