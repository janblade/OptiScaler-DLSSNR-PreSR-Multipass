...
struct Device{FusedBuilder builder;Buffer norm;NVDX_ObjectHandle fused=nullptr;ComPtr<ID3D12Device>owner;std::map<uint64_t,Template> contracts;std::array<Weight,8>w{};Buffer weights;std::vector<ComPtr<ID3D12Resource>>uploads;std::vector<NVDX_ObjectHandle>modules;
...
struct Session{std::array<Blob,8> fusedParams;std::array<FusedInfo,8> fusedInfo{};uint64_t tokens=0;unsigned block=31,pairs=0;bool active=false,pending=false;UINT64 x=0,y=0,done=0;unsigned width=0,height=0;ID3D12Device*device=nullptr;
...
struct State{std::recursive_mutex mutex;bool enabled=false,restartRequired=false;std::string status="FP8 selected";uint64_t launches=0;uint64_t seq=0;std::map<NVDX_ObjectHandle,bool>modules;std::map<NVDX_ObjectHandle,Target>targets;std::map<ID3D12Device*,Device>devices;std::map<std::tuple<ID3D12GraphicsCommandList*,ID3D12Device*,uint64_t>,Session>sessions;
 // Evicted-but-not-provably-idle sessions: kept alive here (never freed under recorded GPU work,
 // per the file's retain-until-exit model) so the s.sessions lookup/scan stays bounded.
...
void Prepare(Device&d,ID3D12Device*device,const NvApi&nv){VerifyAssets();auto&uploadBatch=*new UploadBatch(device);ID3D12GraphicsCommandList*c=uploadBatch.cmd.Get();d.owner=device;auto root=Assets();for(auto&entry:ContractPaths())d.contracts.emplace(entry.first,LoadTemplate(entry.second));
...
void KickDeviceInit(State&s,Device&d,ID3D12Device*device){
 NvApi nv{s.createModule,s.createFunction};
 d.init.store(1,std::memory_order_relaxed);
 device->AddRef();
 std::thread([&s,&d,device,nv]{
  std::string err;
  try{Prepare(d,device,nv);}catch(const std::exception&e){err=e.what();}catch(...){err="unknown hybrid init failure";}
...
void AllocateSession(Session&s,Device&d,ID3D12GraphicsCommandList*c){if(s.allocated)return;auto dev=d.owner.Get();auto&uploadBatch=*new UploadBatch(dev);c=uploadBatch.cmd.Get();const uint64_t P=((s.tokens+127)/128)*128;
 s.xp=Allocate(dev,P*1024/2);s.xs=Allocate(dev,P*1024/16);s.yp=Allocate(dev,P*4096/2);s.ys=Allocate(dev,P*4096/16);s.c=Allocate(dev,P*1024*2);s.cw=Allocate(dev,d.contracts.at(s.tokens).workspaceBytes);
 s.zeroBytes=std::max<uint64_t>(P*4096/2,s.cw.bytes);s.zero=Resource(dev,s.zeroBytes,D3D12_HEAP_TYPE_UPLOAD,D3D12_RESOURCE_STATE_GENERIC_READ);void*p=nullptr;Check(s.zero->Map(0,nullptr,&p));memset(p,0,s.zeroBytes);s.zero->Unmap(0,nullptr);
 for(auto*b:{&s.xp,&s.xs,&s.yp,&s.ys,&s.c,&s.cw})CopyZero(c,*b,s.zero.Get(),b->bytes);uploadBatch.Finish();s.allocated=true;}
...
void Vendor(ID3D12GraphicsCommandList*c,Device&d,Session&s){auto&t=d.contracts.at(s.tokens);auto w=d.w[s.block-31];auto&workspace=s.cw;CopyZero(c,workspace,s.zero.Get(),t.resetBytes);Blob p=t.params;
 ...
 LaunchBlob(c,t.fn,p.data(),(unsigned)p.size(),t.grid,t.block,t.shared);Barrier(c);}
...
NvAPI_Status __cdecl Launch(ID3D12GraphicsCommandList*c,const NVAPI_CU_KERNEL_LAUNCH_PARAMS*k,NvU32 count){auto&s=S();std::lock_guard<std::recursive_mutex>g(s.mutex);
 // FP8 passthrough must stay available even after a latched hybrid failure: check `enabled` first
 // so switching Precision back to 0 fully recovers without an app restart.
 ...
 const auto fallback=[&](){cycle.active=false;s.status="Original FP8 fallback: M="+std::to_string(tokens)+" dims="+std::to_string(dims[0])+"x"+std::to_string(dims[1])+(!t.supported?" unsupported original module hash":(s.devices.count(t.device)&&!s.devices.at(t.device).failure.empty()?" initialization: "+s.devices.at(t.device).failure:" no supplied contraction template"));};
 ...
 if(di!=2){ // device init runs on a worker; record FP8 until it publishes Ready
  if(di==0)KickDeviceInit(s,d,t.device);
  cycle.active=false;
 ...
 struct Pack{uint64_t input,output,sf;unsigned rows,cols;}pack{p[0],cycle.xp.address(),cycle.xs.address(),(unsigned)tokens,1024};IO(c,d.pack,pack,(unsigned)tokens*1024/2);
 auto&blob=cycle.fusedParams[cycle.block-31];auto&info=cycle.fusedInfo[cycle.block-31];
 LaunchBlob(c,d.fused,blob.data(),(unsigned)blob.size(),{info.grid[0],info.grid[1],info.grid[2]},{info.block[0],info.block[1],info.block[2]},(unsigned)info.shared_bytes);Barrier(c);
 ++s.launches;s.status="Fused hybrid M="+std::to_string(tokens)+" dims="+std::to_string(dims[0])+"x"+std::to_string(dims[1])+" expansion block "+std::to_string(cycle.block);return NVAPI_OK;
 }
 ...
 auto&d=s.devices[t.device];Vendor(c,d,cycle);auto w=d.w[cycle.block-31];
 struct Epilogue{uint64_t gemm,skip,cosine,output;unsigned rows,pad;uint64_t done,counter,expand_done;}ep{cycle.c.address(),p[1],d.weights.address()+w.cos,p[2],(unsigned)tokens,0,p[7],p[4],cycle.done};IO(c,d.epilogue,ep,(unsigned)tokens*1024/2);
 cycle.pending=false;++cycle.block;++cycle.pairs;++s.launches;s.status="Fused hybrid M="+std::to_string(tokens)+" dims="+std::to_string(dims[0])+"x"+std::to_string(dims[1])+" FFN pairs recorded "+std::to_string(cycle.pairs)+"/8 ("+std::to_string(cycle.pairs*2)+"/16 original launches); other operators FP8";return NVAPI_OK;
...
void SetEnabled(bool on){auto&s=S();std::lock_guard<std::recursive_mutex>g(s.mutex);
 ...
 // means an already-recorded command list still expects our contraction kernel.
 ...
 if(!on){s.enabled=false;tryRecover();s.status="Original FP8 selected";return;}
 tryRecover();
 if(s.restartRequired){s.status="Restart required: switch Precision to FP8 with no frame in flight, then back, to retry";return;}
 if(s.enabled)return;
 ...
 s.enabled=true;s.status="Hybrid FFN selected; waiting for supported M960/M2160/M3840 original model";}
std::string Status(){auto&s=S();std::lock_guard<std::recursive_mutex>g(s.mutex);return s.status;}
}
