...
struct State{std::recursive_mutex mutex;bool enabled=false,candidate=false,restartRequired=false;std::string status="FP8 selected";uint64_t launches=0,splitLaunches=0;uint64_t seq=0;std::map<NVDX_ObjectHandle,bool>modules;std::map<NVDX_ObjectHandle,Target>targets;std::map<std::pair<ID3D12Device*,bool>,Device>devices;std::map<std::tuple<ID3D12GraphicsCommandList*,ID3D12Device*,uint64_t,bool>,Session>sessions;
...
void SetPrecision(unsigned precision){auto&s=S();std::lock_guard<std::recursive_mutex>g(s.mutex);const bool on=precision==4,candidate=on;
 ...
 if(!on){s.enabled=false;s.candidate=false;tryRecover();s.status="Original FP8 selected";return;}
 tryRecover();
 ...
 s.enabled=on;s.candidate=candidate;s.status=candidate?"Candidate hybrid selected; waiting for original model":"Hybrid FFN selected; waiting for supported original model";
}
