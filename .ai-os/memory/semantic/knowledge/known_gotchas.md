...
- **A PR to wilsjo2 must be cut from his release branch, not from our `main`, and the two lines differ a lot.** His contributors target `codex/release-v0.8.4` (== tag `v0.8.4`). Ours is ~338 commits ahead of it and lacks 54 of its commits, so a branch from `main` shows hundreds of unrelated files (PR #78: 202). On that line the RTX 40 unlock is compile-time opt-in (`/p:OptiScalerRtx40Mfg=true`, `OPTISCALER_RTX40_MFG`), with no Ampere unlock, no `[FrameGen] External` and no `AdaBlackwellKernels`. Its clang-format check already fails on the base branch, so a red check there is not necessarily the PR's; compare against the added lines. Use the GitHub compare (`ahead/behind/files`) before proposing it.
- **Never call swapchain methods (`GetBuffer`, `QueryInterface`) while holding the NR mutexes; check that the feature is enabled first.** On wilsjo2's v0.8.4, `DlssNr_Dx12::State::ApplyToFinishedPicture` did, under `nrOwnersMutex` and `State::mutex`, and deadlocked with the FSR FG present thread (`hkExecuteCommandLists` -> `DlssNr::FinishedPictureSubmitted` needs the same mutex while it holds the swapchain lock). It only bites with OptiFG (FSR FG) and NR both active, even with Finished Picture off. The main-line function has the gate at its top; PR #80 restores it. With Finished Picture on and OptiFG the lock order is still unsafe.
- **A hang shows up in the log as an FG present that starts and never finishes.** Compare `FGHooks::hkFGPresent SyncInterval` with `FGHooks::FGPresent Present finished` counts: they differ by one. Windows logs no crash or hang event for it, and the log is overwritten on every launch, so keep a copy. See the "Live hang" playbook for finding the stuck threads.
