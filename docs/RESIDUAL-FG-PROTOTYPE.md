...
Status: included in the v0.7.0 experimental preview, with standalone hardware tests and known game-integration limitations.
...
The newer [latest-completed NR mode](ASYNC-LATEST-NR.md) overrides this half-rate
interpolation path. It keeps the raster current and accepts stale NR residuals,
instead of delaying the image for a matching interpolated residual.
